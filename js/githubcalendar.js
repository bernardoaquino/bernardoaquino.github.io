function GithubCalendar(git_githubapiurl, git_color, git_user) {
    if (document.getElementById('github_container')) {
        var github_calendar = (git_user, git_githubapiurl, git_color) => {
            var git_fixed = 'fixed';
            var git_px = 'px';
            var git_month = ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'June', 'July', 'Aug', 'Sept', 'Oct', 'Nov', 'Dec'];
            var git_monthchange = [];
            ...
            var git_monthbeforeday = 0;
            var git_monthfirstindex = 0;
            ...
            var responsiveChart = () => {
                if (document.getElementById("gitcanvas")) {
                    ...
                    var linemaxwidth = height / 9;
                    var lineminwidth = 0.8 * linemaxwidth;
                    var setposition = { x: 0.028 * width, y: 0.025 * width };
                    for (var week in git_data) {
                        ...
                        for (var day in weekdata) {
                            ...
                            github_calendar_ctx.fillRect(setposition.x, setposition.y, lineminwidth, lineminwidth);
                    ...
                    if (document.body.clientWidth > 700) {
                        ...
                        // github_calendar_ctx.fillText("Sun", 0, 1.9 * linemaxwidth);
                        github_calendar_ctx.fillText("Mon", 0, 2.9 * linemaxwidth);
                        // github_calendar_ctx.fillText("Tue", 0, 3.9 * linemaxwidth);
                        github_calendar_ctx.fillText("Wed", 0, 4.9 * linemaxwidth);
                        // github_calendar_ctx.fillText("Thu", 0, 5.9 * linemaxwidth);
                        github_calendar_ctx.fillText("Fri", 0, 6.9 * linemaxwidth);
                        // github_calendar_ctx.fillText("Sat", 0, 7.9 * linemaxwidth);
                        var monthindexlist = github_calendar_c.width / 24;
                        for (var index in git_monthchange) {
                            github_calendar_ctx.fillText(git_monthchange[index], monthindexlist, 0.7 * linemaxwidth);
                    ...
                    var getMousePos = (canvas, event) => {
                        ...
                        for (var item of git_positionplusdata) {
                            var lengthx = x - item.x;
                            var lengthy = y - item.y;
                            if (0 < lengthx && lengthx < lineminwidth) {
                                if (0 < lengthy && lengthy < lineminwidth) {
                                    git_span1 = item.date;
            ...
            var tooltip_html = (x, y, span1, span2) => {
                var html = '';
                html += '<div class="gitmessage" style="top:' + y + 'px;left:' + x + 'px;position: fixed;z-index:9999"><div class="angle-wrapper" style="display:block;"><span>' + span1 + '&nbsp;</span><span>' + span2 + ' uploads</span></div></div>';
                return html;
            };
            ...
            var github_info_box = (user, color) => {
                var html = '';
                html += '<div id="git_tooltip_container"></div><div class="contrib-footer clearfix mt-1 mx-3 px-3 pb-1"><div class="float-left text-gray">Data from <a href="https://github.com/' + user + '" target="blank">@' + user + '</a></div><div class="contrib-legend text-gray">Less <ul class="legend"><li style="background-color:' + color[0] + '"></li><li style="background-color:' + color[1] + '"></li><li style="background-color:' + color[2] + '"></li><li style="background-color:' + color[3] + '"></li><li style="background-color:' + color[4] + '"></li></ul>More </div></div>';
                return html;
            };
            ...
            var github_main_box = (monthchange, git_data, user, color, total, thisweekdatacore, weekdatacore, oneyearbeforeday, thisday, aweekago, amonthago) => {
                ...
                html += '<div style="display:flex;width:100%"><div class="contrib-column contrib-column-first table-column"><span class="text-muted">Past year contributions</span><span class="contrib-number">' + total + '</span><span class="text-muted">' + oneyearbeforeday + '&nbsp;-&nbsp;' + thisday + '</span></div><div class="contrib-column table-column"><span class="text-muted">Last month contributions</span><span class="contrib-number">' + thisweekdatacore + '</span><span class="text-muted">' + amonthago + '&nbsp;-&nbsp;' + thisday + '</span></div><div class="contrib-column table-column"><span class="text-muted">Last week contributions</span><span class="contrib-number">' + weekdatacore + '</span><span class="text-muted">' + aweekago + '&nbsp;-&nbsp;' + thisday + '</span></div></div>' + style;
                return html;
            };
        ...
        github_calendar(git_user, git_githubapiurl, git_color);
    }
}